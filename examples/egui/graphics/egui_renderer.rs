...
impl EguiRenderer {
    pub(crate) fn new(vk_init: &VkInit, frames_in_flight: usize) -> Result<Self> {
        let create_info = RendererCreateInfo {
            initial_buffer_length: 1024 * 64,
            frames_in_flight,
            topology: PrimitiveTopology::TRIANGLE_LIST,
            blend_mode: BlendMode::PremultipliedTransparency,
            vertex_code_path: String::from("./assets/egui/shaders/compiled/egui.vert.spv"),
            fragment_code_path: String::from("./assets/egui/shaders/compiled/egui.frag.spv"),
            additional_usage_index_buffer: BufferUsageFlags::empty(),
            additional_usage_vertex_buffer: BufferUsageFlags::empty(),
            debug_name: String::from("VKU_EguiRenderer"),
        };
        let base_renderer =
    ...
    pub(crate) fn destroy(&self, vk_init: &VkInit) -> Result<()> {
    ...
    pub(crate) fn input(
        ...
        let vertices: Vec<egui::epaint::Vertex> = self
            .clipped_primitives
            .iter()
            .filter_map(|clip| match &clip.primitive {
        ...
        let index_buffer = &self.base_renderer.index_buffers[0];
        let vertex_buffer = &self.base_renderer.vertex_buffers[0];
    ...
    pub(crate) fn draw(
        ...
        for (_, cp) in self.clipped_primitives.iter().enumerate() {
            let Primitive::Mesh(mesh) = &cp.primitive else { return Err(anyhow!("Render callbacks are unsupproted"))};
