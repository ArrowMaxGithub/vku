...
impl<'a> VkInit<'a> {
    ...
    pub fn create_compute_shader<Push>(
        ...
        let module_info = ShaderModuleCreateInfo::default().code(&code);
        let module = unsafe { self.device.create_shader_module(&module_info, None) }?;
        self.set_debug_object_name(module, format!("{base_debug_name}_Compute_Shader_Module"))?;
        ...
        let spec_consts_info = SpecializationInfo::default()
        ...
        let push_constants_ranges = [PushConstantRange::default()
            .offset(0)
        ...
        let desc_pool_create_info = DescriptorPoolCreateInfo::default()
            .max_sets(1)
        ...
        self.set_debug_object_name(desc_pool, format!("{base_debug_name}_Descriptor_Pool"))?;
        ...
        for (i, set) in desc_sets.iter().enumerate() {
            self.set_debug_object_name(*set, format!("{base_debug_name}_SSBO_Set_{i}"))?;
        }
        ...
        self.set_debug_object_name(pipeline, format!("{base_debug_name}_Pipeline"))?;
